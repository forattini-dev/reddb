...
impl<'a> KvAtomicOps<'a> {
    ...
    fn resolve_target(&self, key: &str, create_default: bool) -> KvTarget {
        ...
        if let Some((collection, rest)) = key.split_once('.') {
            if !collection.is_empty() && !rest.is_empty() && store.get_collection(collection).is_some()
            {
                return KvTarget {
    ...
    fn is_expired(&self, collection: &str, entity: &crate::storage::UnifiedEntity) -> bool {
        let Some(metadata) = self.runtime.db().store().get_metadata(collection, entity.id) else {
            return false;
        };
        ...
        let Some(ttl_ms) = metadata
            .get("_ttl_ms")
            .and_then(metadata_u64)
            .or_else(|| metadata.get("_ttl").and_then(metadata_u64).and_then(|s| s.checked_mul(1000)))
        else {
            return false;
        };
        entity.created_at.saturating_mul(1000).saturating_add(ttl_ms) <= now_ms
    }
...
mod tests {
    ...
    fn put_get_delete_use_kv_default() {
        ...
        assert!(rt.db().store().get_collection(super::DEFAULT_KV_COLLECTION).is_some());
